...
def train_endpoint(request: TrainRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not dataset:
            metrics_manager.request_count.labels(method="POST", endpoint="/train", http_status="404").inc()
        ...
        if not train_data:
            metrics_manager.request_count.labels(method="POST", endpoint="/train", http_status="400").inc()
        ...
        # Incrémenter les métriques pour une réponse réussie
        metrics_manager.request_count.labels(method="POST", endpoint="/train", http_status="200").inc()
        latency = time.time() - start_time
        metrics_manager.request_latency.labels(method="POST", endpoint="/train", http_status="200").observe(latency)
        ...
        # Incrémenter les métriques pour une réponse échouée
        metrics_manager.request_count.labels(method="POST", endpoint="/train", http_status=str(http_exc.status_code)).inc()
        latency = time.time() - start_time
        metrics_manager.request_latency.labels(method="POST", endpoint="/train", http_status=str(http_exc.status_code)).observe(latency)
        ...
        # Incrémenter les métriques pour une réponse échouée
        metrics_manager.request_count.labels(method="POST", endpoint="/train", http_status="500").inc()
        latency = time.time() - start_time
        metrics_manager.request_latency.labels(method="POST", endpoint="/train", http_status="500").observe(latency)
